...
from capstoneDataVisualization import fetch_ts, engineer_features
...
def _model_train(df,tag,test=False):
    ...
    if test:
        ...
        joblib.dump(pipe_rf, SAVED_MODEL)
    ...
    ## update log
    print("TAG:",tag)
    print("Valores:",str(dates[0]),str(dates[-1]))
    ...
    #update_train_log(X.shape, eval_rmse, runtime,       MODEL_VERSION, MODEL_VERSION_NOTE, test=True)
...
def model_predict(country,year,month,day,all_models=None,test=False):
    ...
    return({'country':country,'y_pred':y_pred,'y_proba':y_proba})

if __name__ == "__main__":

    """
    basic test procedure 
    model_train(DATA_DIR,test=True) to save models
    model_train(DATA_DIR,test=False) when not training I save date to retrain from these
    """
    ...
    all_data, all_models = model_load("sl")
