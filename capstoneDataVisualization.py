...
DATA_DIR = "C:\\Architecture\\IA\\Curso Python\\Capstone\\ai-workflow-capstone-master\\cs-train\\ts-data"
...
def run_data_ingestion(key):
    ...
    df = pd.read_csv(os.path.join(DATA_DIR,"ts-"+key+".csv"))
    print("Columnas data ingestion:",df.columns)
...
def create_plot(df,key):
    """
    create a two panel subplot that summarizes two features of the data with respect to Country and Suscriber Type
    """
    print("Columnas entrada de create plot:",df.columns)

    print("... creating plot")
    columns_to_show = ["revenue","year_month"]
    pd.pivot_table(df, index= 'year_month',aggfunc='sum').round(3)
    ...
    milista=[]
    misetiquetas=[]
    for i in df.Year_Month.cat.categories:
        milista.append(df[df.Year_Month == i].revenue)
        misetiquetas.append(i)
    
#Añadimos una leyenda y un título
    plt.title("Data for " + key, fontsize=15, color="DArkBlue", fontname="Console")
    plt.hist(milista, bins=15, stacked=True, rwidth=1, label=misetiquetas) #rdwith = ratio del ancho del bin  
    plt.legend() #Obligatorio para que muestre la leyenda. Se le pueden añadir paramteros para cambiar aspecto, tamaño
    plt.ylabel("Year_Month", fontsize=25, color='Red')
    plt.xlabel("Revenue", fontsize=25, color='Green')
...
#   data_dir = os.path.join("..","data","cs-train")
    ...
    df=run_data_ingestion("all")
    #df=run_data_ingestion("singapore")
    
    
    print("Df:",df.size)
    create_plot(df,"10 top countries")
    print("....feature engineering...")
    vuelta=engineer_features(df)
    
    #print("Final:",vuelta[0],vuelta[1],vuelta[2])
    print("Final:",vuelta[2].size)
